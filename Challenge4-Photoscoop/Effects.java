...
//UIUC CS125 SPRING 2016 MP. File: Effects.java, CS125 Project: Challenge4-Photoscoop, Version: 2016-02-22T08:07:56-0600.345149194
...
public class Effects {
	/**
	 * Returns a new image (2D array) based on the command and the source
	 * parameters. This method delegates all of the work to PixelEffects class
	 * 
	 * @param cmd
	 *            - the command to execute
	 * @param source
	 *            - the primary image source (not changed)
	 * @param background
	 *            - the secondary (background) image (not changed)
	...
	public static int[][] process(String cmd, int[][] source, int[][] background) {
		...
		// e.g. if(cmd.equals("hide")) return
		// Stenography.hide(source,background);
		// e.g. else if(cmd.equals("extract")) return
		// Stenography.extract(source);
		// And add the others for PixelEffects

		// hint: for some commands the string is slightly different than the
		// method name
		// hint: remember to use string.equals not == to compare strings
		// This message and the exception should not occur if you match all
		// possible command strings

		/*
		 * "half", "rotate", "flip", "mirror","redeye","funky" , "resize",
		 * "merge", "key"
		 */
